...
const ProjectListPage = () => {
    ...
    try {
      ...
      if (response.ok) {
        const updatedProjects = projectData.map(project =>
          project._id === projectId ? { ...project, status: newStatus, updatedAt: new Date() } : project
        );
  ...
  return (
    <Layout title="Project-list" >
      {/* Mobile screen */}
      <div className={`sm:hidden flex justify-between gap-12 mx-3 relative items-center ${showSortOption?'opacity-60':''} `}>
        <input
      ...
      <div  className={`sm:hidden pb-[70px] mx-1 ${showSortOption?'opacity-60':''} `}>
        {filteredProjects.map((project) => (
          <Card key={project._id} project={project} onStatusChange={handleStatusChange} />
        ))}
      </div>

      {
           ...
           <div className="flex w-full  items-center justify-between px-[3px] "  >
            ...
            <p className="text-center text-[25px] text-gray-600 self-center" 
        ...
        className="text-lg"
        >Priority</button>
        <button onClick={() => setSortOption('updatedAt')}
           className="text-lg">Recently Modified</button>
        <button onClick={() => setSortOption('status')}
           className="text-lg"
          >Status</button>
        <button onClick={() => setSortOption('startDate')}
           className="text-lg"
          >Start Date</button>
        <button onClick={() => setSortOption('endDate')}
           className="text-lg"
          >End Date</button>
           </div>
            </div>
